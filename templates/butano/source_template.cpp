...
{tiles_definition}
    ...
    {{
        BN_ASSERT(id < {objects_count}, "Invalid object ID: ", id);
        return {object_getter};
    }}

    const bn::span<const bntmx::map_object> {map_name}::objects(int objects_layer_index) const
    {{
        BN_ASSERT(objects_layer_index < {objects_layers_count}, "Invalid objects layer index: ", objects_layer_index);
    ...
    {{
        BN_ASSERT(objects_layer_index < {objects_layers_count}, "Invalid objects layer index: ", objects_layer_index);
        BN_ASSERT(objects_class < {objects_classes_count}, "Invalid objects class: ", objects_class);
        return {objects_getter_with_class};
    }}

    const bn::span<const bntmx::map_tile> {map_name}::tiles(int tiles_layer_index) const
    {{
        BN_ASSERT(tiles_layer_index < {tiles_layers_count}, "Invalid tiles layer index: ", tiles_layer_index);
        return {tiles_getter};
    }}
}}
