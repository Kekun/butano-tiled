...
def _object_position(object_node: ET.Element) -> tuple[int,int]:
    ...
    x = int(object_node.get("x")) + int(object_node.get("width")) // 2 if object_node.get("width") is not None else int(object_node.get("x"))
    y = int(object_node.get("y")) - int(object_node.get("height")) // 2 if object_node.get("height") is not None else int(object_node.get("y"))
    return x, y
